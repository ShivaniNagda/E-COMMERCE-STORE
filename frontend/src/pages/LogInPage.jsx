...
import { User, UserPlus,Mail,Lock,ArrowRight,Loader, LogIn } from 'lucide-react';
        ...
        transition={{duration: 0.8 , delay: 0.2 }}>
            <div className="bg-gray-800 py-8 px-4 shadow sm:rounded-lg sm:px-10">
              ...
              whileHover={{scale:1.02}} whileTap={{scale:0.98}} type="submit" disabled={loading}>
                ...
                {loading ? (<><Loader className="animate-spin mr-2" size={24}></Loader>Loading...</>):(<><LogIn className='mr-2 h-5 w-5' aria-hidden='true' />LogIn</>)}</button>  
                </form>
                </div>
          ...
          <p className="text-sm text-gray-400">
            Not a member?{" "}
            <Link to={"/signup"} className="text-green-400 hover:underline" >Sign up now <ArrowRight className='inline h-4 w-4' /></Link>
          </p>
        </div>
        </motion.div>
    </div>
